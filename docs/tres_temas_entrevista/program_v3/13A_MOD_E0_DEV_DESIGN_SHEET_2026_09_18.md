# 13A — MOD-E0-DEV: numerical design sheet (RP2), sealed `96f447e6…`
...
| period P_A | 24 samples | P_A = 24 samples gives 86 cycles in training and 2 cycles in the context; small enough for many cycles, large enough for the detector's receptive field (17) to see a fraction of a cycle | [12, 48] |
...
| r = 0 control | phantom partner: an independent trajectory with group A's parameters feeds the same β-lagged term | the lagged partner term is replaced by an independent N(0,1) draw scaled to the same variance; marginals preserved in distribution (ML04 checks variance, ACF and quantiles across replicates); no temporal shuffle | — |
...
6312; H3 sequence 920 vs summary
