...
| Comparador multiescala principal | MTST; Pathformer; TimeMixer | Elegir uno después de reproducibilidad y costo en E1 | Ejecutar los tres puede dispersar recursos | Cambia el vecino directo, no la definición del método. |
| Comparador de agrupación | CCM; ablación propia solamente | CCM si puede integrarse sin cambiar información | Es el vecino más directo de grupos de canales | Si no se integra, la reivindicación frente a agrupación debe estrecharse. |
...
1. ¿CCM, REP-Net u otro trabajo posterior ya usa propiedades train-only para decidir simultáneamente grupos y campos receptivos?
