...
| D2-R4 AT9 portability | `IMPLEMENTED` + **replay executed on the three roles** | `d2_support_r1/r4/R4_SNR_MARGINS.json` (1,026 decisions vs every threshold), `R4_ENVIRONMENT_INVENTORY.json` (coordinator role), `R4_DIAGNOSTIC_SUBSET.json` (16 regimes, 32 units, fixed rule; ≤6 CPU-h, ≤4 h wall, 2 GiB/process); tolerance unchanged. Replay: `tools/df_d2_r4_replay.py` from a diagnostic copy of each role's own tools, same `df_snr.py` digest `a5ad155f…`, 403 CPU-seconds in total, ≤191 MB peak. Result (`R4_PORTABILITY_POLICY.md`, `R4_PORTABILITY_SUMMARY.json`): **four** estimators exact on the three CPUs (`mad_first_difference`, `wavelet_mad`, `spectral_floor`, `trailing_median_residual`, max |Δ| = 0.0 dB) and a fifth, `ar_residual`, inside the observed tolerance but not exact (1.918e-13 dB) — corrected 2026-09-14; only `local_level_kalman` deviates — ≤2.44e-05 dB on confirmation, 1.099 dB on one near-flat calibration cell — with 0 identifiability changes and **0 of 288 decisions changed** under a comparator that now declares its own coverage (32/32 units, 1,716 cells, 639 substituted facts, 16/16 regimes; verdict `MEASURED`) after the empty-input defect the owner found — PRE 7/7 failing → POST 7/7 in `d2_support_r1/r4_guard/`; COORDINATOR and WORKER_A are bit-identical in both directions, WORKER_B is the outlier with identical package versions. AT9 stays open under its original criterion |
