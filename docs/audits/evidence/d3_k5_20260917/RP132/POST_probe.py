"""Musashi's RP131 probe, re-run against the RP132/RP133 repairs.

This file is HIS probe, byte for byte, with ONE adaptation: the revalidation accessors. RP133 was ordered to stop reporting
`deletion_eligible_today` (a second, weaker destructive gate) and to stop calling locally recorded evidence currently accepted,
so `cell["deletion_eligible_today"]` and `summary["with_full_numeric_acceptance"]` no longer exist. His unchanged probe therefore
raises `KeyError: 'deletion_eligible_today'` at that line — after the certificate and deletion cases have already run — and that
refusal is recorded verbatim beside this file in UNCHANGED_PROBE_REFUSAL.txt. Nothing else is changed: the two certificate
counterexamples, the fixtures, the thermal guard and the real deletion API calls are his.
"""
import argparse
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="musashi-rp131-") as temp:
        ...
        def revalidate(warehouse):
            ...
            cell = r["cells"][unit]
            return {"summary": r["summary"], "mode": r["mode"], "status": r["status"],
                    "accepted": cell["catalog_acceptance"]["accepted"]["accepted"],
                    "current_cell_verified": cell["cell_verified_by_current_closure"],
                    "eligible": cell["deletion_preview"]["state"] == "CONDITIONAL_PREVIEW_ELIGIBLE",
                    "preview_state": cell["deletion_preview"]["state"],
                    "currently_accepted_numeric_evidence": cell["currently_accepted_numeric_evidence"],
                    "locally_recorded_numeric_evidence": cell["locally_recorded_numeric_evidence"],
                    "closure_changed_since_acceptance": cell.get("closure_changed_since_acceptance")}
