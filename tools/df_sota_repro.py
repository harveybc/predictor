...
def accept_regenerated(root: Path, design: dict, unit: str, *, data_path: Path, delete_after: bool = False) -> dict:
    ...
    # RP117 on a deleted cell's evidence: with the regenerated bytes in hand, the estimator families of the RETAINED catalog are
    # also checked against a second implementation that never calls the producer
    ind = independent_estimators(preds, trues)
    g_ret = (retained or {}).get("global") or {}
    res_ret = (retained or {}).get("residuals") or {}
    comp = {"residual_sd": ind["residual_sd"] - (res_ret.get("sd") or 0.0), "residual_mean": ind["residual_mean"] - (res_ret.get("mean") or 0.0),
            "entropy_bits": (ind["entropy_bits"] - res_ret["entropy_bits"]) if (ind["entropy_bits"] is not None and res_ret.get("entropy_bits") is not None) else 0.0,
            "mutual_information_bits": (ind["mutual_information_bits"] - g_ret["mutual_information_bits_pred_true_64x64"])
            if (ind["mutual_information_bits"] is not None and g_ret.get("mutual_information_bits_pred_true_64x64") is not None) else 0.0,
            "corr_pred_true": (ind["corr_pred_true"] - g_ret["corr_pred_true"]) if (ind["corr_pred_true"] is not None and g_ret.get("corr_pred_true") is not None) else 0.0,
            "r2": (ind["r2"] - g_ret["r2"]) if (ind["r2"] is not None and g_ret.get("r2") is not None) else 0.0,
            "mae": ind["mae_float64"] - (g_ret.get("mae") or 0.0), "mse": ind["mse_float64"] - (g_ret.get("mse") or 0.0)}
    out["independent_estimators"] = ind; out["independent_vs_retained_catalog"] = comp
    out["independent_scope"] = ("recomputed from the REGENERATED arrays (bit-identical to the deleted originals) by a second implementation that never calls "
                                "the catalog's producer; this lifts the historical limit for this cell")
    for k, tol in (("residual_sd", 1e-6), ("residual_mean", 1e-6), ("entropy_bits", 1e-6), ("mutual_information_bits", 1e-6),
                   ("corr_pred_true", 1e-6), ("r2", 1e-6), ("mae", 1e-6), ("mse", 1e-6)):
        if abs(comp[k]) > tol:
            out["refusals"].append(f"INDEPENDENT {k}: the retained catalog disagrees with an independent recomputation by {comp[k]}")
    out["pass"] = not out["refusals"]
