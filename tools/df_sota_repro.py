...
def accept_regenerated(root: Path, design: dict, unit: str, *, data_path: Path, delete_after: bool = False) -> dict:
    ...
    # RP117/RP124 on a deleted cell's evidence: with the regenerated bytes in hand, every declared family of the RETAINED catalog
    # is checked against the independent implementation, field by field, with explicit coverage
    _, loader2 = DF.data_provider(args, "test")
    ...
    out["deleted"] = []
    # RP126: the temporaries go when the acceptance passed, and also when the regeneration is provably NOT the deleted original
    # (those bytes certify nothing and are reproducible from the checkpoint at the same cost); they are kept when bytes that claim
    # to be identical fail some other check, because that case is worth inspecting.
    provably_other = reg.get("identity") != "BIT_IDENTICAL_TO_THE_DELETED_ORIGINAL"
    if delete_after and (out["pass"] or provably_other):
        ...
        out["reading"] = (("the regenerated temporaries are removed after this acceptance; " if out["pass"] else
                           "the acceptance REFUSED and the regenerated bytes are not the deleted original, so the temporaries are removed as well; ") +
                          "the retained catalog, record, checkpoint and this acceptance record remain, and the regeneration can be repeated from the "
                          "checkpoint at the same cost")
...
CATALOG_ESTIMATORS = {
    # RP124: the finite approved catalog, declared BEFORE measurement. Each family names the population it reduces, the axis and
    # orientation of that reduction, its parameters, the condition under which it is undefined, and the tolerance an independent
    # implementation of the SAME definition must meet. A field absent from this inventory is not claimed to be independently checked.
    "errors": {"fields": ["global.mse", "global.mae", "global.rmse", "global.bias"], "population": "every window x step x channel",
               "orientation": "residual = pred - true", "reduction": "float64 mean over all elements (rmse = sqrt(mse))", "undefined": None, "tolerance": 1e-9},
    "percentage_errors_zspace": {"fields": ["global.mape_zspace", "global.mspe_zspace"], "population": "elements with |true| > 1e-8",
                                 "orientation": "|(true - pred)/true|", "reduction": "float64 mean over the retained elements",
                                 "undefined": "no element with |true| > 1e-8", "tolerance": 1e-6},
    "matched_baselines": {"fields": ["global.naive_mse", "global.naive_mae", "global.seasonal24_mse", "global.seasonal24_mae",
                                     "global.skill_mae_vs_naive", "global.skill_mse_vs_naive"],
                          "population": "the same windows, from the author's loader inputs", "orientation": "persistence = last input value repeated; seasonal24 = the value 24 steps before, wrapped inside the input window",
                          "reduction": "float64 mean over all elements", "undefined": "input length < 24 disables seasonal24", "tolerance": 1e-9},
    "residual_moments": {"fields": ["residuals.mean", "residuals.var", "residuals.sd", "residuals.skewness", "residuals.kurtosis_raw"],
                         "population": "every residual element", "orientation": "pred - true",
                         "reduction": "central moments about the mean; skewness = m3/sd^3, kurtosis_raw = m4/sd^4", "undefined": "zero residual variance", "tolerance": 1e-6},
    "entropy": {"fields": ["residuals.entropy_bits"], "population": "the 2000-bin residual histogram on [-20, 20]",
                "orientation": "Shannon entropy in bits of the empirical bin distribution", "reduction": "-sum p log2 p over occupied bins",
                "parameters": {"bins": 2000, "range": [-20.0, 20.0]}, "undefined": "empty histogram", "tolerance": 1e-9},
    "quantiles": {"fields": ["residuals.quantiles"], "population": "the same 2000-bin histogram",
                  "orientation": "the centre of the first bin whose cumulative share reaches q", "reduction": "searchsorted on the histogram CDF, NOT an exact order statistic",
                  "parameters": {"bins": 2000, "bin_width": 0.02, "q": [0.001, 0.01, 0.05, 0.1, 0.25, 0.5, 0.75, 0.9, 0.95, 0.99, 0.999]},
                  "undefined": "empty histogram", "tolerance": 1e-9},
    "correlation_r2": {"fields": ["global.corr_pred_true", "global.r2", "global.true_var", "global.pred_var", "global.true_mean", "global.pred_mean"],
                       "population": "every element", "orientation": "Pearson between pred and true; r2 = 1 - mse/var(true)",
                       "reduction": "float64 sums", "undefined": "zero variance on either side", "tolerance": 1e-6},
    "mutual_information": {"fields": ["global.mutual_information_bits_pred_true_64x64"], "population": "the 64x64 joint histogram of (pred, true) clipped to [-6, 6]",
                           "orientation": "Shannon MI in bits", "reduction": "sum p log2 (p / (px py))", "parameters": {"bins": 64, "range": [-6.0, 6.0]},
                           "undefined": "empty joint histogram", "tolerance": 1e-9},
    "autocorrelation": {"fields": ["autocorrelation.channel_mean_residual_per_step.acf_by_lag", "autocorrelation.per_channel"],
                        "population": "for each FIXED forecast step, the series of channel-mean residuals over window origins",
                        "orientation": "centred per step over origins; denominator = sum of squared deviations over all origins",
                        "reduction": "sum_{w} s[w+lag] s[w] / sum_w s[w]^2, per step and per lag", "parameters": {"max_lag": 168, "channel_lags": [1, 24, 168]},
                        "undefined": "zero variance over origins for that step or channel, or fewer origins than the lag", "tolerance": 1e-9},
    "per_step_series": {"fields": ["per_step.mse", "per_step.mae", "per_step.bias", "per_step.naive_mae", "per_step.seasonal24_mae"],
                        "population": "all windows and channels at each forecast step", "orientation": "pred - true",
                        "reduction": "float64 mean over windows x channels", "undefined": None, "tolerance": 1e-9},
    "per_channel_series": {"fields": ["per_channel.mse", "per_channel.mae", "per_channel.bias", "per_channel.r2", "per_channel.corr_pred_true"],
                           "population": "all windows and steps for each channel", "orientation": "pred - true",
                           "reduction": "float64 mean over windows x steps", "undefined": "zero variance for r2 and correlation", "tolerance": 1e-6},
    "per_window_series": {"fields": ["per_window.mae", "per_window.mse"], "population": "all steps and channels of each window",
                          "orientation": "pred - true", "reduction": "float64 mean over steps x channels", "undefined": None, "tolerance": 1e-9},
    "time_blocks": {"fields": ["time_blocks"], "population": "consecutive blocks of 168 windows", "orientation": "means of the per-window series",
                    "reduction": "float64 mean over the block's windows", "parameters": {"block_windows": 168}, "undefined": None, "tolerance": 1e-9},
    "histogram": {"fields": ["residuals.histogram.counts", "residuals.histogram.outside_range"], "population": "every residual element",
                  "orientation": "pred - true", "reduction": "counts per bin", "parameters": {"bins": 2000, "range": [-20.0, 20.0]}, "undefined": None, "tolerance": 0.0},
}
...
def _acf_over_origins(series: np.ndarray, lags: int) -> list:
    """ACF of each COLUMN of `series` (one column per forecast step or per channel) over its rows (window origins), centred per
    column, denominator = the column's total squared deviation. Independent implementation of the declared definition."""
    out = []
    centred = series - series.mean(axis=0, keepdims=True)
    den = np.einsum("wc,wc->c", centred, centred)
    for lag in range(1, lags + 1):
        num = np.einsum("wc,wc->c", centred[lag:], centred[:-lag])
...
def independent_estimators(preds, trues, *, test_loader=None, pred_len: int | None = None, block_windows: int = 168,
    ...
    supplies them in the same order; without it the baseline and seasonal families are reported UNCHECKED rather than skipped
    silently."""
    W, T, C = preds.shape
    n = W * T * C
    S = {k: np.zeros((T, C)) for k in ("ab", "sq", "r", "nab", "nsq", "sab", "ssq", "y", "y2", "p", "p2", "py")}
    s1 = s2 = s3 = s4 = 0.0
    edges = np.linspace(-20.0, 20.0, 2001); hist = np.zeros(2000); outside = 0
    jedges = np.linspace(-6.0, 6.0, 65); joint = np.zeros((64, 64))
    ...
    pct_n = 0; pct_abs = 0.0; pct_sq = 0.0
    tail = {"1": 0, "2": 0, "3": 0}
    baselines_checked = test_loader is not None
    seasonal_supported = None
    w0 = 0
    batches = iter(test_loader) if test_loader is not None else None
    step = max(1, (64 << 20) // max(1, T * C * 8))
    while w0 < W:
        if batches is not None:
            batch = next(batches)
            bx = batch[0]; by = batch[1]
            x = (bx.numpy() if hasattr(bx, "numpy") else np.asarray(bx)).astype(np.float64)
            b = x.shape[0]
        else:
            b = min(step, W - w0); x = None
        a = np.asarray(preds[w0:w0 + b], dtype=np.float64); y = np.asarray(trues[w0:w0 + b], dtype=np.float64)
        d = a - y                                                    # pred - true, the declared orientation
        S["ab"] += np.abs(d).sum(0); S["sq"] += (d * d).sum(0); S["r"] += d.sum(0)
        S["y"] += y.sum(0); S["y2"] += (y * y).sum(0); S["p"] += a.sum(0); S["p2"] += (a * a).sum(0); S["py"] += (a * y).sum(0)
        flat = d.reshape(-1)
        s1 += flat.sum(); s2 += np.dot(flat, flat); s3 += float((flat ** 3).sum()); s4 += float((flat ** 4).sum())
        h, _ = np.histogram(flat, bins=edges); hist += h; outside += int(flat.size - h.sum())
        joint += np.histogram2d(np.clip(a.reshape(-1), -6, 6), np.clip(y.reshape(-1), -6, 6), bins=[jedges, jedges])[0]
        keep = np.abs(y) > 1e-8
        if keep.any():
            rel = (y[keep] - a[keep]) / y[keep]
            pct_n += int(keep.sum()); pct_abs += float(np.abs(rel).sum()); pct_sq += float((rel * rel).sum())
        for k in tail:
            tail[k] += int((np.abs(flat) > float(k)).sum())
        ...
        w0 += b
    mse = float(S["sq"].sum()) / n; mae = float(S["ab"].sum()) / n; bias = float(S["r"].sum()) / n
    mean = s1 / n; var = s2 / n - mean * mean; sd = math.sqrt(max(0.0, var))
    m3 = s3 / n - 3 * mean * (s2 / n) + 2 * mean ** 3
    m4 = s4 / n - 4 * mean * (s3 / n) + 6 * mean * mean * (s2 / n) - 3 * mean ** 4
    total = hist.sum()
    ph = hist / total if total > 0 else None
    entropy = float(-(ph[ph > 0] * np.log2(ph[ph > 0])).sum()) if ph is not None else None
    cdf = np.cumsum(hist) / total if total > 0 else None
    centers = (edges[:-1] + edges[1:]) / 2
    quantiles = ({str(q): float(centers[min(len(centers) - 1, int(np.searchsorted(cdf, q)))]) for q in q_list} if cdf is not None else None)
    pj = joint / max(1.0, joint.sum()); pa, pb = pj.sum(1, keepdims=True), pj.sum(0, keepdims=True); nz = pj > 0
    mi = float((pj[nz] * np.log2(pj[nz] / (pa @ pb)[nz])).sum())
    yy, y2, pp, p2, py = float(S["y"].sum()), float(S["y2"].sum()), float(S["p"].sum()), float(S["p2"].sum()), float(S["py"].sum())
    sst = y2 - yy * yy / n; ssp = p2 - pp * pp / n
    corr = float((py - pp * yy / n) / math.sqrt(ssp * sst)) if ssp > 0 and sst > 0 else None
    r2 = float(1 - (float(S["sq"].sum())) / sst) if sst > 0 else None
    lags = max(0, min(max_lag, W - 2))
    acf = _acf_over_origins(step_series, lags) if lags > 0 else []
    ch_acf = {}
    for where, arr in (("first_step", ch_first), ("last_step", ch_last)):
        ch_acf[where] = {str(l): (_acf_over_origins(arr, l)[-1] if l < W - 1 else None) for l in (1, 24, 168)}
    WT = W * T
    blocks = []
    for b0 in range(0, W, block_windows):
        b1 = min(W, b0 + block_windows)
        blocks.append({"windows": [b0, b1], "n": b1 - b0, **{k: float(per_window[k][b0:b1].mean()) for k in per_window}})
    out = {"population": {"windows": W, "steps": T, "channels": C, "elements": n},
           "global": {"mse": mse, "mae": mae, "rmse": math.sqrt(mse), "bias": bias,
                      "mape_zspace": (pct_abs / pct_n) if pct_n else None, "mspe_zspace": (pct_sq / pct_n) if pct_n else None,
                      "rse": math.sqrt(float(S["sq"].sum()) / sst) if sst > 0 else None, "r2": r2, "corr_pred_true": corr,
                      "true_mean": yy / n, "pred_mean": pp / n, "true_var": sst / n, "pred_var": ssp / n,
                      "mutual_information_bits_pred_true_64x64": mi},
           "residuals": {"mean": mean, "var": var, "sd": sd, "skewness": (m3 / sd ** 3) if sd > 0 else None,
                         "kurtosis_raw": (m4 / sd ** 4) if sd > 0 else None, "entropy_bits": entropy, "quantiles": quantiles,
                         "fraction_abs_gt": {k: v / n for k, v in tail.items()},
                         "histogram": {"counts": hist.astype(int).tolist(), "outside_range": outside}},
           ...
           "per_channel": {"mse": (S["sq"].sum(0) / WT).tolist(), "mae": (S["ab"].sum(0) / WT).tolist(), "bias": (S["r"].sum(0) / WT).tolist(),
                           "r2": [float(1 - sq / (yv - ys ** 2 / WT)) if (yv - ys ** 2 / WT) > 0 else None for sq, yv, ys in zip(S["sq"].sum(0), S["y2"].sum(0), S["y"].sum(0))],
                           "corr_pred_true": [_corr(py_, pp_, yy_, p2_, y2_, WT) for py_, pp_, yy_, p2_, y2_ in zip(S["py"].sum(0), S["p"].sum(0), S["y"].sum(0), S["p2"].sum(0), S["y2"].sum(0))]},
           "per_window": {k: v.tolist() for k, v in per_window.items() if k in ("mae", "mse")},
           "time_blocks": blocks,
           "autocorrelation": {"channel_mean_residual_per_step": {"lags": list(range(1, lags + 1)), "acf_by_lag": acf}, "per_channel": ch_acf},
           "baselines_checked": baselines_checked, "seasonal_supported": seasonal_supported,
           "definitions": {k: {kk: vv for kk, vv in v.items() if kk != "fields"} for k, v in CATALOG_ESTIMATORS.items()}}
    if baselines_checked:
        naive_mae_ = float(S["nab"].sum()) / n; naive_mse_ = float(S["nsq"].sum()) / n
        seas_mae_ = float(S["sab"].sum()) / n; seas_mse_ = float(S["ssq"].sum()) / n
        out["global"].update({"naive_mae": naive_mae_, "naive_mse": naive_mse_, "seasonal24_mae": seas_mae_, "seasonal24_mse": seas_mse_,
                              "skill_mae_vs_naive": (1 - mae / naive_mae_) if naive_mae_ > 0 else None,
                              "skill_mse_vs_naive": (1 - mse / naive_mse_) if naive_mse_ > 0 else None})
        out["per_step"].update({"naive_mae": (S["nab"].sum(1) / (W * C)).tolist(), "seasonal24_mae": (S["sab"].sum(1) / (W * C)).tolist()})
        out["per_window"].update({k: per_window[k].tolist() for k in ("naive_mae", "naive_mse", "seasonal24_mae", "seasonal24_mse")})
    # the exact order statistics, as a SEPARATE labelled check of the declared histogram approximation (never substituted for it)
    out["exact_quantiles_note"] = "not computed: the declared estimator is the histogram CDF reading, and the independent reference uses that same declared definition"
    return out


def _dig(obj, path: str):
    """`a.b.c` lookup that distinguishes ABSENT from a present None."""
    cur = obj
...
def _numeric_diff(a, b):
    ...
    if a is None or b is None:
        return (None, f"DISAGREEMENT: one side is undefined and the other is {b if a is None else a!r}")
    if isinstance(a, dict) and isinstance(b, dict):
        if set(a) != set(b):
            return (None, f"DISAGREEMENT: keys differ ({sorted(set(a) ^ set(b))[:4]})")
        worst, status = 0.0, "OK"
        for k in a:
            d, st = _numeric_diff(a[k], b[k])
            if d is None:
                return (None, f"{st} at {k}")
            worst = max(worst, d)
            if st != "OK":
                status = st
        return (worst, status)
    if isinstance(a, (list, tuple)) and isinstance(b, (list, tuple)):
        if len(a) != len(b):
            return (None, f"DISAGREEMENT: lengths differ ({len(a)} vs {len(b)})")
        worst, status = 0.0, "OK"
        for x, y_ in zip(a, b):
            d, st = _numeric_diff(x, y_)
            if d is None:
                return (None, st)
            worst = max(worst, d)
            if st != "OK":
    ...
    if isinstance(a, (int, float)) and isinstance(b, (int, float)):
        if not math.isfinite(float(a)) or not math.isfinite(float(b)):
            return (None, "DISAGREEMENT: a non-finite value")
        return (abs(float(a) - float(b)), "OK")
...
def compare_catalog_to_reference(vault: dict, ind: dict) -> dict:
    ...
    out = {"fields": {}, "coverage": {}, "disagreements": [], "unchecked": []}
    for family, spec in CATALOG_ESTIMATORS.items():
        checked, skipped = [], []
        for field in spec["fields"]:
            have_v, cat = _dig(vault, field)
            have_i, ref = _dig(ind, field)
            if have_v == "ABSENT" or have_i == "ABSENT":
                out["fields"][field] = {"status": "UNCHECKED", "why": f"catalog {have_v.lower()}, reference {have_i.lower()}"}
                out["unchecked"].append(field); skipped.append(field); continue
            diff, status = _numeric_diff(cat, ref)
            tol = spec.get("tolerance", 1e-9)
            ok = diff is not None and diff <= tol
            out["fields"][field] = {"status": ("OK" if ok else "DISAGREEMENT"), "max_abs_difference": diff, "tolerance": tol, "detail": status,
                                    "undefined_declared": spec.get("undefined")}
            checked.append(field)
            if not ok:
                out["disagreements"].append(f"{field}: {status if diff is None else f'max |difference| {diff} exceeds {tol}'}")
        out["coverage"][family] = {"checked": checked, "unchecked": skipped, "complete": not skipped}
    out["families_complete"] = [f for f, c in out["coverage"].items() if c["complete"]]
    out["families_incomplete"] = [f for f, c in out["coverage"].items() if not c["complete"]]
    out["fully_independent"] = not out["disagreements"] and not out["families_incomplete"]
    return out
...
def accept_catalog(root: Path, design: dict, unit: str, *, data_path: Path | None = None, independent: bool = True) -> dict:
    ...
    # the same definitions, compared field by field under the family's declared tolerance, with explicit coverage
    arrays = folder / "arrays.npz"
    out["declared_estimators"] = {k: {kk: vv for kk, vv in v.items() if kk != "fields"} for k, v in CATALOG_ESTIMATORS.items()}
    if independent and arrays.is_file() and data_path is not None:
        ...
        author_env()
        args = build_args(cell["argv"], data_dir=Path(data_path).parent, data_name=Path(data_path).name, checkpoints=Path("/nonexistent"), gpu=0, use_gpu=False)
        args.augmentation_ratio = 0
        DF = importlib.import_module("data_provider.data_factory")
        _, loader = DF.data_provider(args, "test")
        ...
        out["independent_estimators"] = None; out["independent_comparison"] = None; out["independently_accepted_families"] = []
