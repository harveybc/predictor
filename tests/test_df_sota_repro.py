...
def test_RP112_a_regeneration_is_accepted_only_when_its_catalog_and_oracles_reproduce_the_retained_one_then_cleaned_up(world, tmp_path):
    ...
    assert acc["independent_estimators"]["elements"] == acc["population"]["elements"]
    assert all(abs(v) <= 1e-6 for v in acc["independent_vs_retained_catalog"].values()) and "lifts the historical limit" in acc["independent_scope"]
