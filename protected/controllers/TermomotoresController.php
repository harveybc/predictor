...
function arreglarUltimo($modelIn) {
    ...
    if (!isset($motor->plan_mant_termografia)) {
        echo '<script language="JavaScript">
                        alert("Atención: NO se generó aviso ZI para esta medición porque el motor no tiene plan de mantenimiento de termografia, por favor ingrese el plan de mantenimiento..");
                     </script>';
        return(0);
    }
    if ($motor->plan_mant_termografia == 0) {
        echo '<script language="JavaScript">
                        alert("Atención: NO se generó aviso ZI para esta medición porque el motor no tiene plan de mantenimiento de termografia, por favor ingrese el plan de mantenimiento..");
                    </script>';
        return(0);
    }
    $modelAviso = AvisosZI::model()->findBySql('select * from avisosZI where plan_mant=' . $motor->plan_mant_termografia . ' AND arreglado=0 order by Fecha desc');
...
function crearAviso($modelIn) {
    ...
    $modelAviso->Ruta = "/index.php/termomotores/" . $modelIn->id;
    ...
    $modelAviso->plan_mant = $motor->plan_mant_termografia;
    if (!$modelAviso->save())
        echo '<script language="JavaScript">
                            alert("Error guardando el aviso ZI.");
                        </script>';
...
class TermomotoresController extends Controller {
    ...
    public function actionCreate() {
        $model = new Termomotores;
        $modelArchivo = new Archivos;
        $this->performAjaxValidation($model);
        $continuar = 1;
        if (isset($_POST['Termomotores'])) {
            $model->attributes = $_POST['Termomotores'];
            $modelArchivo->nombre = $model->Path;
            $modelArchivo->save(false);
            $model->Path = "" . $modelArchivo->id;
            if ($model->save()) {

                crearEvento('TERMOMOTORES', 'CREAR', 'Medición creada');
        ...
        if (isset($_GET['id'])) {
            ...
            if (isset($motor)) {
                if (!isset($motor->plan_mant_termografia)) {
                    ...
                    $motor->plan_mant_termografia=$motor->id+15000000;
                    // se guarda el modelo
                    $motor->save();
                    $continuar = 1;
                } else
    ...
    public function actionUpdate() {
        ...
        if (isset($_POST['Termomotores'])) {
            ...
            if ($model->save()) {
                ...
                crearEvento('TERMOMOTORES', 'ACTUALIZAR', 'Medición actualizada');
                $this->redirect(array('view', 'id' => $model->id));
            }
        }
    ...
    public function actionDelete() {
        if (Yii::app()->request->isPostRequest) {
            ...
            crearEvento('TERMOMOTORES', 'BORRAR', 'Medición borrada');
            if (!isset($_GET['ajax']))
    ...
    public function actionIndex() {
        $dataProvider = new CActiveDataProvider('Termomotores');
    ...
    public function actionAdmin() {
        ...
        if (isset($_GET['Termomotores']))
            $model->attributes = $_GET['Termomotores'];
    ...
    public function loadModel() {
        if ($this->_model === null) {
            if (isset($_GET['id']))
                $this->_model = Termomotores::model()->findbyPk($_GET['id']);
            if ($this->_model === null)
    ...
    protected function performAjaxValidation($model) {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'termomotores-form') {
