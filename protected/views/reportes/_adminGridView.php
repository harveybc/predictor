...
$this->widget('zii.widgets.grid.CGridView', array(
    ...
    'columns' => array(
        array(
            'header'=>'Imagen',
            'type'=>'raw',
            'value' => 'sprintf(\'<a href="/index.php/%s"><img src="/index.php/%s" style="height:120px;width:120px;border-width:1px;" /></a>\',is_numeric($data->Path)?("archivos/displayArchivo?id=".$data->Path):("reportes/passthru?path=".urlencode($data->Path)), is_numeric($data->Path)?("archivos/displayArchivo?id=".$data->Path):("reportes/passthru?path=".urlencode($data->Path))  )',
        ),
        array(
            'header'=>'Estado',
            'type'=>'raw',
