            ...
            <td style="width:50%;">
                ...
                echo CHtml::dropDownList(
                        'MetaDocs[UT_Proceso]', $valor, CHtml::listData(Estructura::model()->findAllbySql(
                                        isset($modeloId) ?
                                                'SELECT DISTINCT Area FROM estructura WHERE Proceso="' . $valor . '" ORDER BY Area ASC' : 'SELECT DISTINCT Area FROM estructura WHERE Proceso="ELABORACION" ORDER BY Area ASC', array()), 'Area', 'Area'
                        ), array(
                ...
                <div id="divTableros" >
                    <?php
                        ...
                        echo CHtml::activeDropDownList($modelMeta,
                            'UT_Tablero_TAG', CHtml::listData(Tableros::model()->findAllbySql(
                                    'SELECT TAG,concat(TAG," - ",Tablero) as Tablero FROM tableros WHERE Area="' .$valor. '" ORDER BY Tablero ASC',
                                                                                                    array ()),
                                                                                                    'TAG',
                                                                                                    'Tablero'
                            ),
                        ...
                        if (isset($modeloId))
                            $modelMeta->UT_Tablero_TAG=isset($modeloId->TAG)?$modeloId->TAG:"";
                    ?>
                </div>
            </td>
        </tr>
