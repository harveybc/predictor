               ...
               <table style="padding-left:10px">
                    <tr>
                        <td>
                            <b>Area:</b>
                            <?php
                            $valor = isset($modeloId) ? isset($modeloId->Proceso) ? $modeloId->Proceso : "ELABORACION" : "ELABORACION";
// dibuja el dropDownList de Proceso, seleccionando los valores diferentes presentes en la tabla Estructura col. Proceso
                            echo CHtml::dropDownList(
                                ...
                                'style' => 'width:406px;margin-right:10px',
                                'class' => 'select'
                                    )
                            );
                            ?>
                        ...
                        <td>
                            ...
                            echo CHtml::dropDownList(
                                    'MetaDocs[UT_Proceso]', $valor, CHtml::listData(Estructura::model()->findAllbySql(
                                                    isset($modeloId) ?
                                                            'SELECT DISTINCT Area FROM estructura WHERE Proceso="' . $valor . '" ORDER BY Area ASC' : 'SELECT DISTINCT Area FROM estructura WHERE Proceso="ELABORACION" ORDER BY Area ASC'
                                                    , array()), 'Area', 'Area'
                                    ), array(
                                //'onfocus' => 'updateFieldArea()',
                            ...
                            //echo $modeloId;
                            $valor = isset($modeloId) ? isset($modeloId->Area) ? $modeloId->Area : "" : "";
                            ?>
                        </td>
                    </tr>
                    </table>
                        ...
                        <td>
                            ...
                            echo CHtml::dropDownList(
                                    'MetaDocs[UT_Equipo]', $valor, CHtml::listData(Estructura::model()->findAllbySql(
                                                    isset($modeloId) ?
                                                            'SELECT Equipo FROM estructura WHERE Area="' . $valor . '" ORDER BY Equipo ASC' : 'SELECT Equipo FROM estructura WHERE Area="FILTRACION" ORDER BY Equipo ASC', array()), 'Equipo', 'Equipo'
                                    ), array(
                                //'onfocus' => 'updateFieldEquipo()',
                                'onchange' => 'updateFieldMotor()',
                               'style' => 'width:406px;margin-right:2px',
                                'class' => 'select',
                                'empty' => 'Seleccione el equipo para filtrar el resultado',
                                    )
                            );
                            $valor = isset($modeloId) ? isset($modeloId->Equipo) ? $modeloId->Equipo : "" : "";
                            ?>
                        </td>
                        <td>
                            <b>Motor:<br/></b>
...
// dibuja el dropDownList de Proceso, seleccionando los valores diferentes presentes en la tabla Estructura col. Proceso
                            echo CHtml::dropDownList(
                                    'MetaDocs[UT_Motor_TAG]', $valor, CHtml::listData(Motores::model()->findAllbySql(
                                                    isset($modeloId) ?
                                                            'SELECT TAG, CONCAT(TAG," - ",Motor) as Motor FROM motores WHERE Equipo="' . $valor . '" ORDER BY TAG ASC' : 'SELECT TAG, CONCAT(TAG," - ",Motor) as Motor FROM motores WHERE Equipo="ANILLO DE CONTRAPRESION" ORDER BY TAG ASC', array()), 'TAG', 'Motor'
                                    ), array(
                                //'onfocus' => 'updateGraph()',
