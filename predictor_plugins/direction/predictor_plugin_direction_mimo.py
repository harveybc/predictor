#!/usr/bin/env python
"""Direction MIMO predictor plugin.

Thin subclass of the regression MIMO plugin for direction classification.
Reuses the MIMO encoder (Conv1D stack + BiLSTM + GlobalAveragePooling)
as the shared trunk and adds a single Dense(1, sigmoid) direction output.
"""
...
class Plugin(DirectionMixin, MIMOPlugin):
    plugin_params = {
    ...
    def build_model(self, input_shape, x_train, config):
        ...
        self.model = Model(
            ...
            name=f"DirectionMIMO_{signal_type}",
        )
        self.model.compile(
            optimizer=AdamW(
