#!/usr/bin/env python
"""Binary N-BEATS predictor plugin (deterministic).

Thin subclass of the regression N-BEATS plugin.  Replicates the
N-BEATS block stack (backcast/forecast) and replaces the per-horizon
Dense output heads with a single Dense(1, sigmoid) binary head.

Deterministic: predict_with_uncertainty returns zero uncertainties
...
class Plugin(BinaryMixin, NBeatsPlugin):
    plugin_params = {
    ...
    def build_model(self, input_shape, x_train, config):
        ...
        # -- Trunk (identical to regression N-BEATS) --
        ...
        self.model = Model(
            ...
            name=f"BinaryNBEATS_{signal_type}",
        )
        self.model.compile(
            optimizer=AdamW(
