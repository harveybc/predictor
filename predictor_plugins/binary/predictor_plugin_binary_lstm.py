#!/usr/bin/env python
"""Binary LSTM predictor plugin.

The regression LSTM plugin is a standalone class (does not inherit from
the common base classes), so this binary version inherits from
``BinaryMixin`` + ``BaseBayesianKerasPredictor`` directly and replicates
the LSTM trunk architecture:

    Positional Encoding → MultiHeadAttention + Residual + LayerNorm
    → AveragePooling → BiLSTM × 2 → AveragePooling → trunk

Then adds a single Dense(1, sigmoid) binary output head.
"""
...
class Plugin(BinaryMixin, BaseBayesianKerasPredictor):
    plugin_params = {
        "batch_size": 32,
        "learning_rate": 0.001,
        "activation": "relu",
    ...
    def build_model(self, input_shape, x_train, config):
        ...
        # -- Trunk: PE → MHA → AvgPool → BiLSTM × 2 → AvgPool --
        ...
        self.model = Model(
            ...
            name=f"BinaryLSTM_{signal_type}",
        )
        self.model.compile(
            optimizer=AdamW(
