#!/usr/bin/env python
"""Binary TCN predictor plugin (deterministic).

Thin subclass of the regression TCN plugin.  Replicates the dilated
causal Conv1D backbone and replaces the per-horizon output heads with a
single Dense(1, sigmoid) binary head.
...
class Plugin(BinaryMixin, TCNPlugin):
    ...
    def build_model(self, input_shape, x_train, config):
        ...
        # -- Trunk (identical to regression TCN) --
