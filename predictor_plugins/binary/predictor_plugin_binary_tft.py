#!/usr/bin/env python
"""Binary TFT predictor plugin (deterministic).

Thin subclass of the regression TFT plugin.  Replicates the GRN-based
encoder (LSTM + MultiHeadAttention) and replaces the per-horizon output
heads with a single Dense(1, sigmoid) binary head.
...
class Plugin(BinaryMixin, TFTPlugin):
    plugin_params = {
