#!/usr/bin/env python
"""Binary ANN predictor plugin.

Thin subclass of the regression ANN plugin.  Replicates the shared trunk
(Flatten → Dense stack with Dropout) and replaces the multi-horizon
DenseFlipout output heads with a single Dense(1, sigmoid) binary head.
"""
...
class Plugin(BinaryMixin, ANNPlugin):
    ...
    def build_model(self, input_shape, x_train, config):
        ...
        # -- Trunk (identical to regression ANN) --
        ...
        self.model = Model(
            inputs=inputs, outputs=[output], name=f"BinaryANN_{signal_type}"
        )
        self.model.compile(
            optimizer=AdamW(
