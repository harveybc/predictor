...
class HeuristicStrategy(bt.Strategy):
    ...
    This strategy uses pre‐computed future predictions from a CSV file to decide on trade entry.
    When no position is open, it looks at the daily predictions (columns starting with 'Prediction_d_')
    to compute the ideal profit and drawdown (in pips) relative to the current price.
    
    It then calculates a risk–reward ratio (RR) and, if the predicted profit exceeds a threshold, chooses the
    trade direction (long or short) with the higher RR. The TP and SL levels are set by applying configurable
    multipliers to the ideal profit and drawdown.
    
    Order size is determined by interpolating between a minimum and maximum volume based on the RR, capped
    by available cash.
    
    While in a trade, on every bar the strategy checks both the hourly and daily predictions (columns starting
    with 'Prediction_h_' and 'Prediction_d_') to decide if the trade should be closed early.
    
    Trade frequency is limited to a maximum number of trades per rolling 5‐day period.
    
    At the end of the simulation, a summary of trade statistics is printed and a balance-versus-date plot is saved.
    """
    ...
    params = (
        ...
        ('rel_volume', 0.05),           # Max fraction of cash to risk.
        ('min_order_volume', 10000),    # Minimum order volume (in currency units).
        ('max_order_volume', 1000000),  # Maximum order volume (in currency units).
        ('leverage', 1000),             # Leverage.
        ('profit_threshold', 5),        # Minimum ideal profit (pips) required for entry.
    ...
    def __init__(self):
        ...
        # Price feed from the first data feed.
        ...
        # For tracking the intra-trade extreme prices:
        self.trade_low = None  # For long trades: lowest price reached since entry.
        self.trade_high = None  # For short trades: highest price reached since entry.
    ...
    def next(self):
        ...
        # If a position is open, update intra-trade extreme and check for exit.
        if self.position:
            if self.order_direction == 'long':
                # Update the lowest price seen since entry.
                if self.trade_low is None or current_price < self.trade_low:
                    self.trade_low = current_price
                # Exit conditions: if current price reaches TP or the predicted future minimum is below the SL.
                ...
                if current_price <= self.current_tp or predicted_max > self.current_sl:
                    self.close()
                    return
            return  # Do not check for new entries if position is open.
        ...
        # Check for prediction data for the current bar.
        ...
        ideal_drawdown_pips_buy = ((current_price - min(daily_preds)) / self.p.pip_cost
                                    if current_price > min(daily_preds) else self.p.min_drawdown_pips)
        ...
        ideal_drawdown_pips_sell = ((max(daily_preds) - current_price) / self.p.pip_cost
                                     if current_price < max(daily_preds) else self.p.min_drawdown_pips)
        ...
        # Determine qualifying signals based on profit threshold.
        ...
        # Also, record the entry price (will be updated in notify_order)
    ...
    def compute_size(self, rr):
        """Compute order size by linearly interpolating between min and max volumes based on RR."""
    ...
    def stop(self):
        """Print summary statistics and plot balance vs date."""
...
if __name__ == '__main__':
    ...
    # Dummy sizer since order size is computed by the strategy.
