...
def run_prediction_pipeline(config, plugin):
    ...
    for iteration in range(1, iterations + 1):
        ...
        train_mmd = plugin.compute_mmd(train_preds , y_train, sigma=1.0, sample_size=mc_samples)
        val_mmd = plugin.compute_mmd(val_preds, y_val, sigma=1.0, sample_size=mc_samples)
        test_mmd = plugin.compute_mmd(test_predictions, y_test, sigma=1.0, sample_size=mc_samples)
        # add to the lists
