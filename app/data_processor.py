...
def run_prediction_pipeline(config, plugin):
    ...
    if config.get("use_normalization_json") is not None:
        ...
        # ------------------------------
    # NEW: Plot Last 1k Predictions with True Values and Uncertainty Band (excluding weekends)
    # ------------------------------
    try:
        n_plot = 1000
        # If test_dates is provided, convert to DatetimeIndex and filter out weekends
        if test_dates is not None:
            if not isinstance(test_dates, pd.DatetimeIndex):
                test_dates = pd.to_datetime(test_dates)
            # Create a boolean mask: True for weekdays (Monday=0,..., Friday=4)
            weekday_mask = test_dates.weekday < 5
            # Filter predictions, uncertainty, and dates
            test_predictions = test_predictions[weekday_mask]
            uncertainty_estimates = uncertainty_estimates[weekday_mask]
            test_dates = test_dates[weekday_mask]
            print(f"DEBUG: Filtered out weekends. Remaining dates: {len(test_dates)}")
        ...
        if test_predictions.shape[0] > n_plot:
            ...
            true_plot = y_test[-n_plot:, 0]             # true values for first time step
            dates_plot = test_dates[-n_plot:]
            uncertainty_plot = uncertainty_estimates[-n_plot:, 0]
        else:
            pred_plot = test_predictions[:, 0]
            true_plot = y_test[:, 0]
            dates_plot = test_dates if test_dates is not None else np.arange(test_predictions.shape[0])
            uncertainty_plot = uncertainty_estimates[:, 0]
        
        # (Assuming that true values in y_test have already been denormalized if needed.)
        # Create the plot
        ...
        plt.title("Last 1000 Weekday Predictions vs True Values with Uncertainty")
        plt.xlabel("Time")
        plt.ylabel("CLOSE")
        plt.legend()
        plt.grid(True)
