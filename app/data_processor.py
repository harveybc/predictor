...
def process_data(config):
    ...
    if config.get("use_daily", False):
        y_train_ma = y_train.rolling(window=12, center=True, min_periods=1).mean()
        y_val_ma = y_val.rolling(window=12, center=True, min_periods=1).mean()
        y_test_ma = y_test.rolling(window=12, center=True, min_periods=1).mean()
