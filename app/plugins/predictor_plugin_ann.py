...
def gaussian_kernel_sum(x, y, sigma, chunk_size=8):
    ...
    def body(i, total):
        ...
        tf.print("DEBUG: gaussian_kernel_sum: processing chunk from i =", i, "to", end_i, "x_chunk shape =", tf.shape(x_chunk))
        ...
        tf.print("DEBUG: gaussian_kernel_sum: chunk_sum =", chunk_sum)
        total += chunk_sum
...
def mmd_loss_term(y_true, y_pred, sigma, chunk_size=16):
    ...
    tf.print("DEBUG: In mmd_loss_term: reshaped y_true shape =", tf.shape(y_true),
             "y_pred shape =", tf.shape(y_pred))
    ...
    tf.print("DEBUG: In mmd_loss_term: m =", m, "n =", n)
    
    mmd = sum_K_xx / (m * m) + sum_K_yy / (n * n) - 2 * sum_K_xy / (m * n)
    tf.print("DEBUG: In mmd_loss_term: mmd =", mmd)
    return mmd
