...
class Plugin:
    ...
    def build_model(self, input_shape):
        ...
        for size in layers[:-1]:
            if size > 1:
                x = LSTM(size, activation='relu', recurrent_activation='relu', kernel_initializer=HeNormal(), return_sequences=True)(x)
                #x = BatchNormalization()(x)
        x = LSTM(layers[-2], activation='relu', recurrent_activation='relu', kernel_initializer=HeNormal())(x)
        model_output = Dense(layers[-1], activation='relu', kernel_initializer=HeNormal(), name="model_output")(x)
