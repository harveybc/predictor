#!/usr/bin/env python
"""
Phase 2.6 Pipeline Plugin - For Pre-processed Z-Score Normalized Data

This pipeline plugin is designed to work with data that has already been:
...
Key differences from STL pipeline:
...
# --- Denormalization Functions for Z-Score Normalization ---
def denormalize(data, config):
    """Denormalizes z-score normalized data using normalization config."""
    ...
    if config.get("use_normalization_json"):
        ...
        if isinstance(norm_json, dict) and "CLOSE" in norm_json:
            try:
                ...
                print(f"WARN: Z-score denorm error: {e}")
                return data
    return data

def denormalize_returns(data, config):
    """Denormalizes z-score normalized returns/deltas."""
    data = np.asarray(data)
    if config.get("use_normalization_json"):
        norm_json = config["use_normalization_json"]
        if isinstance(norm_json, str):
        ...
        if isinstance(norm_json, dict) and "CLOSE" in norm_json:
            try:
                # For returns/deltas, only scale by std (no mean shift)
                close_std = norm_json["CLOSE"]["std"]
                return data * close_std
            except Exception as e:
                print(f"WARN: Z-score denorm returns error: {e}")
                return data
    return data


class Phase26PipelinePlugin:
    # Default parameters optimized for phase 2.6 preprocessed data
    plugin_params = {
    ...
    def set_params(self, **kwargs):
        ...
        config = self.params
        # No additional parameter resolution needed for preprocessed data
    ...
    def run_prediction_pipeline(self, config, predictor_plugin, preprocessor_plugin):
        ...
        print(f"Phase 2.6 Pipeline - Iterations: {iterations}")
        ...
        # --- Save Final Test Outputs (Separate Files) ---
        ...
        try:
            ...
            plt.title(f"Phase 2.6 Predictions vs Target/Actual (H={plotted_horizon})")
            plt.xlabel("Time")
            plt.ylabel("Price")
            plt.legend()
            plt.grid(True, alpha=0.6)
        ...
        if hasattr(predictor_plugin, 'save') and callable(predictor_plugin.save):
            save_model_file = config.get("save_model", "phase2_6_model.keras")
            try:
        ...
        print(f"\nTotal Phase 2.6 Pipeline Execution Time: {time.time() - start_time:.2f} seconds")

    def load_and_evaluate_model(self, config, predictor_plugin, preprocessor_plugin):
        """Load a pre-trained model and evaluate it on validation data."""
        ...
        try:
            ...
            evaluate_filename = config.get('output_file', 'phase2_6_eval_predictions.csv')
