...
#: PostgreSQL has JSONB; DuckDB has JSON and no alias. The loader is shared by both engines,
#: so the type is chosen per dialect rather than duplicated into a second loader that drifts.
def _for_dialect(sql: str, engine) -> str:
    return sql.replace("JSONB", "JSON") if engine.dialect.name == "duckdb" else sql


def _inserted(result) -> int:
    """Rows an INSERT actually wrote, on every engine this loader runs on.

    `rowcount` is -1 on DuckDB for `INSERT … ON CONFLICT DO NOTHING`, so a counter built on it
    reported "-1 units" for a successful envelope. The statements RETURN their key instead.
    """
    try:
...
def ensure_envelope_tables(engine) -> None:
    with engine.begin() as conn:
        conn.exec_driver_sql(_for_dialect(ENVELOPE_DDL, engine))
...
def load_envelope(engine, doc: dict) -> dict:
    ...
    with engine.begin() as conn:
        ...
        for u in doc.get("units", []):
            res = conn.execute(text(_for_dialect(f"""
            ...
            for item in items:
                ...
                """), {
                    ...
                    "es": str(item.get("eligibility_state",
                                       UNAVAILABLE))})
                counts["consumption"] += _inserted(res)
    return counts
